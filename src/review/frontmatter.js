...
function parse(src) {
  ...
  const kept = rows.filter(
    (r) =>
      r.value !== "" ||
      r.items.length > 0 ||
      !rows.some((o) => o !== r && o.key.startsWith(r.key + ".")),
  );
...
function renderTable(src, escape) {
  ...
  if (!rows) {
    return (
      '<div class="ir-block ir-frontmatter" data-line="0">' +
  ...
  const body = rows
    .map(
      (r) =>
        `<tr><th scope="row">${escape(r.key)}</th><td>${renderCell(r, escape)}</td></tr>`,
    )
    .join("");
  return (
    '<div class="ir-block ir-frontmatter" data-line="0">' +
  ...
  md.block.ruler.before(
    ...
    (state, startLine, endLine, silent) => {
      ...
      for (let i = 1; i < close; i++)
        body.push(state.src.slice(state.bMarks[i], state.eMarks[i]));
...
export { install, parse, renderTable };
