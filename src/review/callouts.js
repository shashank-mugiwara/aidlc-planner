...
function install(md, enabled) {
  ...
  md.renderer.rules.ir_callout_open = (tokens, idx) => {
    ...
    const head =
      `<span class="ir-callout-icon" aria-hidden="true">${type.icon}</span>` +
      `<span class="ir-callout-title">${label}</span>`;
    if (fold) {
      return (
        `<details class="${cls} ir-callout-fold"${fold === "+" ? " open" : ""}>` +
        `<summary>${head}</summary><div class="ir-callout-body">`
      );
    }
    return (
      `<div class="${cls}"><div class="ir-callout-head">${head}</div>` +
